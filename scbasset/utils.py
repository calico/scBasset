...
def make_h5(input_ad, input_bed, input_fasta, out_file, seq_len=1344, train_ratio=0.9):
    ...
    Returns:
        None.           Save a h5 file to 'out_file'. X: 1-hot encoded feature matrix. Y: peak by cell matrix. train_ids: data indices used for train. val_ids: data indices used for val. test_ids: data indices unused during training, can be used for test.
    """
    # generate 1-hot-encoding from bed
    seqs_dna, seqs_coords = make_bed_seqs(
        input_bed, fasta_file=input_fasta, seq_len=seq_len
    )
    ...
    test_val_ids = np.random.choice(
        ids, int(len(ids) * (1 - train_ratio)), replace=False
    )
...
def make_model(bottleneck_size, n_cells, seq_len=1344, show_summary=True):
    ...
    current = sequence
    current, reverse_bool = StochasticReverseComplement()(current)  # enable random rv
...
def imputation_Y(X, model):
    ...
    Returns:
        array:          a peak*cell imputed accessibility matrix. Sequencing depth isn't corrected for.  
    """
